...
当通过 XML 来传送数据时，自然会涉及到 XML 的解析工作，通常 Python 可以通过如下三种方式来解析 XML：
    ...
    SAX：该方式将逐行扫描 XML 文档，边扫描边解析，占用内存较小，速度较快，缺点是不能像 DOM 方式那样长期留驻在内存，数据不是长久的，事件过后，
    若没保存数据，数据会丢失。
...
f = open('../../PythonStudy/test.xml', 'w', encoding='utf-8')
# 保存
f.write(new_str)
f.close()
...
# 读取文件
dom = parse('../../PythonStudy/test.xml')
# 获取文档元素对象
...
tree = et.parse('../../PythonStudy/test.xml')
# 根节点
root = tree.getroot()
for stu in root:
